...
The program analyses the content of the vmapcmd.log file created by [b] quadcore compiled 
...
If QuadCore is compiled in debug mode it will then write the vmapcmd.log file. The file does only 
...
1.	Apply the patch to QuadCore to modify your .announce, .gmoff and .gmon commands
2.	Compile QuadCore in debug mode
3.	Go to the position where you suspect a problem
4.	Use .gmon to be sure you will not be attacked when you login
5.	Save, Logoff and stop quadcore
6.	Delete the vmapcmd.log from the QuadCore dir. The logger will append to that file.
7.	Start QuadCore
...
1. Start the vmap debugger with the path to the QuadCore data dir and the full path (name) of 
the log file
...
run: vmapdebugger.exe <your QuadCore data dir> vmapcmd.log
