...
bool ChatHandler::HandleCooldownCommand(const char* args)
{
    ...
    else
    {
        ...
        {
            PSendSysMessage(LANG_UNKNOWN_SPELL, target==m_session->GetPlayer() ? GetQuadCoreString(LANG_YOU) : tNameLink.c_str());
            SetSentErrorMessage(true);
            return false;
        }
        ...
        PSendSysMessage(LANG_REMOVE_COOLDOWN, spell_id, target==m_session->GetPlayer() ? GetQuadCoreString(LANG_YOU) : tNameLink.c_str());
   }
    return true;
}
...
bool ChatHandler::HandleRespawnCommand(const char* /*args*/)
{
    ...
    QuadCore::RespawnDo u_do;
    QuadCore::WorldObjectWorker<QuadCore::RespawnDo> worker(pl,u_do);
...
/// Define the 'Message of the day' for the logon
