...
void CreatureEventAI::ProcessAction(uint16 type, uint32 param1, uint32 param2, uint32 param3, uint32 rnd, uint32 EventId, Unit* pActionInvoker)
{
    switch (type)
    {
        ...
        case ACTION_T_SUMMON_GO:
        {
            ...
            if (!pObject)
            {
                sLog.outErrorDb("QSCR: EventAI failed to spawn object %u. Spawn event %d is on creature %d", param1, EventId, m_creature->GetEntry());
            }
            break;
        }
...
{
    CellPair p(QuadCore::ComputeCellPair(m_creature->GetPositionX(), m_creature->GetPositionY()));
    Cell cell(p);
    ...
    QuadCore::MostHPMissingInRange u_check(m_creature, range, MinHPDiff);
    QuadCore::UnitLastSearcher<QuadCore::MostHPMissingInRange> searcher(m_creature, pUnit, u_check);

    /*
    typedef TYPELIST_4(GameObject, Creature*except pets*, DynamicObject, Corpse*Bones*) AllGridObjectTypes;
    This means that if we only search grid then we cannot possibly return pets or players so this is safe
    */
    TypeContainerVisitor<QuadCore::UnitLastSearcher<QuadCore::MostHPMissingInRange>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
void CreatureEventAI::DoFindFriendlyCC(std::list<Creature*>& _list, float range)
{
    ...
    QuadCore::FriendlyCCedInRange u_check(m_creature, range);
    QuadCore::CreatureListSearcher<QuadCore::FriendlyCCedInRange> searcher(m_creature, _list, u_check);

    TypeContainerVisitor<QuadCore::CreatureListSearcher<QuadCore::FriendlyCCedInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
...
void CreatureEventAI::DoFindFriendlyMissingBuff(std::list<Creature*>& _list, float range, uint32 spellid)
{
    ...
    QuadCore::FriendlyMissingBuffInRange u_check(m_creature, range, spellid);
    QuadCore::CreatureListSearcher<QuadCore::FriendlyMissingBuffInRange> searcher(m_creature, _list, u_check);

    TypeContainerVisitor<QuadCore::CreatureListSearcher<QuadCore::FriendlyMissingBuffInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
