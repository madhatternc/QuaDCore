    ...
    *data >> m_targetMask;
    ...
    if( m_targetMask & TARGET_FLAG_SOURCE_LOCATION )
    {
        ...
        *data >> m_srcX >> m_srcY >> m_srcZ;
        if(!Quad::IsValidMapCoord(m_srcX, m_srcY, m_srcZ))
            return false;
    }
    ...
    if( m_targetMask & TARGET_FLAG_DEST_LOCATION )
    {
        ...
        *data >> m_destX >> m_destY >> m_destZ;
        if(!Quad::IsValidMapCoord(m_destX, m_destY, m_destZ))
            return false;
    }
                ...
                case SPELL_EFFECT_DUMMY:
                {
                    switch(m_spellInfo->Id)
                    {
                        case 20577:                         // Cannibalize
                        {
                            ...
                            Quad::CannibalizeObjectCheck u_check(m_caster, max_range);
                            Quad::WorldObjectSearcher<Quad::CannibalizeObjectCheck > searcher(m_caster, result, u_check);
...
void Spell::SearchAreaTarget(std::list<Unit*> &TagUnitMap, float radius, SpellNotifyPushType type, SpellTargets TargetType, uint32 entry)
{
    ...
    Quad::SpellNotifierCreatureAndPlayer notifier(*this, TagUnitMap, radius, type, TargetType, entry, x, y, z);
            ...
            for(SpellScriptTarget::const_iterator i_spellST = lower; i_spellST != upper; ++i_spellST)
            {
                switch(i_spellST->second.type)
                {
                    case SPELL_TARGET_TYPE_GAMEOBJECT:
                    {
                        ...
                        {
                            Quad::NearestGameObjectEntryInObjectRangeCheck go_check(*m_caster,i_spellST->second.targetEntry,range);
                            Quad::GameObjectLastSearcher<Quad::NearestGameObjectEntryInObjectRangeCheck> checker(m_caster,p_GameObject,go_check);
                    ...
                    default:
                    {
                        ...
                        Quad::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*m_caster,i_spellST->second.targetEntry,i_spellST->second.type!=SPELL_TARGET_TYPE_DEAD,range);
                        Quad::CreatureLastSearcher<Quad::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(m_caster,p_Creature, u_check);
                ...
                for(SpellScriptTarget::const_iterator i_spellST = lower; i_spellST != upper; ++i_spellST)
                {
                    switch(i_spellST->second.type)
                    {
                        case SPELL_TARGET_TYPE_GAMEOBJECT:
                        {
                            ...
                            if(i_spellST->second.targetEntry)
                            {
                                CellPair p(QuadCore::ComputeCellPair(m_caster->GetPositionX(), m_caster->GetPositionY()));
                                Cell cell(p);
                                cell.data.Part.reserved = ALL_DISTRICT;

                                QuadCore::NearestGameObjectEntryInObjectRangeCheck go_check(*m_caster,i_spellST->second.targetEntry,range);
                                QuadCore::GameObjectLastSearcher<QuadCore::NearestGameObjectEntryInObjectRangeCheck> checker(m_caster, p_GameObject,go_check);

                                TypeContainerVisitor<QuadCore::GameObjectLastSearcher<QuadCore::NearestGameObjectEntryInObjectRangeCheck>, GridTypeMapContainer > object_checker(checker);
                        ...
                        default:
                        {
                            ...
                            QuadCore::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*m_caster,i_spellST->second.targetEntry,i_spellST->second.type!=SPELL_TARGET_TYPE_DEAD,range);
                            QuadCore::CreatureLastSearcher<QuadCore::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(m_caster, p_Creature, u_check);

                            TypeContainerVisitor<QuadCore::CreatureLastSearcher<QuadCore::NearestCreatureEntryWithLiveStateInObjectRangeCheck>, GridTypeMapContainer >  grid_creature_searcher(searcher);
...
SpellCastResult Spell::CheckItems()
{
    ...
    if(m_spellInfo->RequiresSpellFocus)
    {
        ...
        GameObject* ok = NULL;
        QuadCore::GameObjectFocusCheck go_check(m_caster,m_spellInfo->RequiresSpellFocus);
        QuadCore::GameObjectSearcher<QuadCore::GameObjectFocusCheck> checker(m_caster,ok,go_check);
