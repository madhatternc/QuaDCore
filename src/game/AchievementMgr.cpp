...
void AchievementMgr::SendAchievementEarned(AchievementEntry const* achievement)
{
    ...
    {
        QuadCore::AchievementChatBuilder say_builder(*GetPlayer(), CHAT_MSG_GUILD_ACHIEVEMENT, LANG_ACHIEVEMENT_EARNED,achievement->ID);
        QuadCore::LocalizedPacketDo<QuadCore::AchievementChatBuilder> say_do(say_builder);
        guild->BroadcastWorker(say_do,GetPlayer());
    }

    if(achievement->flags & (ACHIEVEMENT_FLAG_LOGON_FIRST_KILL|ACHIEVEMENT_FLAG_LOGON_FIRST_REACH))
    {
        // broadcast logon first reached
    ...
    {
        CellPair p = QuadCore::ComputeCellPair(GetPlayer()->GetPositionX(), GetPlayer()->GetPositionY());

        Cell cell(p);
        cell.data.Part.reserved = ALL_DISTRICT;
        cell.SetNoCreate();

        QuadCore::AchievementChatBuilder say_builder(*GetPlayer(), CHAT_MSG_ACHIEVEMENT, LANG_ACHIEVEMENT_EARNED,achievement->ID);
        QuadCore::LocalizedPacketDo<QuadCore::AchievementChatBuilder> say_do(say_builder);
        QuadCore::PlayerDistWorker<QuadCore::LocalizedPacketDo<QuadCore::AchievementChatBuilder> > say_worker(GetPlayer(),sWorld.getConfig(CONFIG_LISTEN_RANGE_SAY),say_do);
        TypeContainerVisitor<QuadCore::PlayerDistWorker<QuadCore::LocalizedPacketDo<QuadCore::AchievementChatBuilder> >, WorldTypeMapContainer > message(say_worker);
...
bool AchievementMgr::IsCompletedCriteria(AchievementCriteriaEntry const* achievementCriteria, AchievementEntry const* achievement)
{
    ...
    if(achievement->flags & (ACHIEVEMENT_FLAG_LOGON_FIRST_REACH | ACHIEVEMENT_FLAG_LOGON_FIRST_KILL))
    {
        // someone on this logon has already completed that achievement
        if(achievementmgr.IsLogonCompleted(achievement))
            return false;
    }
...
void AchievementMgr::CompletedAchievement(AchievementEntry const* achievement)
{
    ...
    // don't insert for ACHIEVEMENT_FLAG_LOGON_FIRST_KILL since otherwise only the first group member would reach that achievement
    // TODO: where do set this instead?
    if(!(achievement->flags & ACHIEVEMENT_FLAG_LOGON_FIRST_KILL))
        achievementmgr.SetLogonCompleted(achievement);
...
void AchievementGlobalMgr::LoadCompletedAchievements()
{
    ...
    if(!result)
    {
        ...
        sLog.outString();
        sLog.outString(">> Loaded 0 logon completed achievements . DB table `character_achievement` is empty.");
        return;
    }
    ...
    sLog.outString();
    sLog.outString(">> Loaded %lu logon completed achievements.",(unsigned long)m_allCompletedAchievements.size());
}
