...
bool LoginQueryHolder::Initialize()
{
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADTUTORIALS,       "SELECT tut0,tut1,tut2,tut3,tut4,tut5,tut6,tut7 FROM character_tutorial WHERE account = '%u' AND logonid = '%u'", GetAccountId(), logonID);
...
{
    // keys can be non cleared if player open logon list and close it by 'cancel'
...
void WorldSession::HandleCharCreateOpcode( WorldPacket & recv_data )
{
    ...
    QueryResult *resultacct = LoginDatabase.PQuery("SELECT SUM(numchars) FROM logoncharacters WHERE acctid = '%d'", GetAccountId());
    if ( resultacct )
    {
    ...
    if ( result )
    {
        ...
        if (charcount >= sWorld.getConfig(CONFIG_CHARACTERS_PER_LOGON))
        {
    ...
    uint32 heroic_free_slots = sWorld.getConfig(CONFIG_HEROIC_CHARACTERS_PER_LOGON);
    ...
    bool AllowTwoSideAccounts = !sWorld.IsPvPLogon() || sWorld.getConfig(CONFIG_ALLOW_TWO_SIDE_ACCOUNTS) || GetSecurity() > SEC_PLAYER;
    ...
    LoginDatabase.PExecute("DELETE FROM logoncharacters WHERE acctid= '%d' AND logonid = '%d'", GetAccountId(), logonID);
    LoginDatabase.PExecute("INSERT INTO logoncharacters (numchars, acctid, logonid) VALUES (%u, %u, %u)",  charcount, GetAccountId(), logonID);
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder * holder)
{
    ...
    if(sWorld.IsFFAPvPLogon() && !pCurrChar->isGameMaster() && !pCurrChar->HasFlag(PLAYER_FLAGS,PLAYER_FLAGS_RESTING) )
