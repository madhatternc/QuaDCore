    ...
    // Loading xp per level data
    {
        ...
        do
        {
            ...
            if(current_level >= sWorld.getConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                ...
                {
                    sLog.outDetail("Unused (> MaxPlayerLevel in quadcore.conf) level %u in `player_xp_for_levels` table, ignoring.",current_level);
                    ++count;                                // make result loading percent "expected" correct in case disabled detail mode for example.
                }
                continue;
            }
    ...
    do
    {
        ...
        switch(tmp.command)
        {
            case SCRIPT_COMMAND_TALK:
            {
                ...
                // if(!objmgr.GetQuadCoreStringLocale(tmp.dataint)) will checked after db_script_string loading
                break;
            }
...
void ObjectMgr::LoadPointsOfInterest()
{
    ...
    do
    {
        ...
        if(!QuadCore::IsValidMapCoord(POI.x,POI.y))
        {
...
static LanguageType GetLogonLanguageType(bool create)
{
    switch(sWorld.getConfig(CONFIG_LOGON_ZONE))
    {
        case LOGON_ZONE_UNKNOWN:                            // any language
        case LOGON_ZONE_DEVELOPMENT:
        case LOGON_ZONE_TEST_SERVER:
        case LOGON_ZONE_QA_SERVER:
            return LT_ANY;
        case LOGON_ZONE_UNITED_STATES:                      // extended-Latin
        case LOGON_ZONE_OCEANIC:
        case LOGON_ZONE_LATIN_AMERICA:
        case LOGON_ZONE_ENGLISH:
        case LOGON_ZONE_GERMAN:
        case LOGON_ZONE_FRENCH:
        case LOGON_ZONE_SPANISH:
            return LT_EXTENDEN_LATIN;
        case LOGON_ZONE_KOREA:                              // East-Asian
        case LOGON_ZONE_TAIWAN:
        case LOGON_ZONE_CHINA:
            return LT_EAST_ASIA;
        case LOGON_ZONE_RUSSIAN:                            // Cyrillic
            return LT_CYRILLIC;
        default:
...
{
    if(strictMask==0)                                       // any language, ignore logon
    {
    ...
    if(strictMask & 0x2)                                    // logon zone specific
    {
        LanguageType lt = GetLogonLanguageType(create);
...
bool ObjectMgr::LoadQuadStrings(DatabaseType& db, char const* table, int32 min_value, int32 max_value)
{
    ...
    for(QuadStringLocaleMap::iterator itr = mQuadStringLocaleMap.begin(); itr != mQuadStringLocaleMap.end();)
    {
        ...
        {
            QuadStringLocaleMap::iterator itr2 = itr;
            ++itr;
            mQuadStringLocaleMap.erase(itr2);
        }
        else
            ++itr;
    }
    ...
    sLog.outString();
    if(min_value == MIN_QUAD_STRING_ID)               // internal Quad strings
...
bool LoadQuadStrings(DatabaseType& db, char const* table,int32 start_value, int32 end_value)
{
    ...
    return objmgr.LoadQuadStrings(db,table,end_value,start_value);
}
