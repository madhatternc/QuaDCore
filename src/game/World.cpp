...
World::AddSession_ (WorldSession* s)
{
    ...
    if (pLimit > 0)
    {
        ...
        popu /= pLimit;
        popu *= 2;
        LoginDatabase.PExecute ("UPDATE logonlist SET population = '%f' WHERE id = '%d'", popu, logonID);
...
void World::LoadConfigSettings(bool reload)
{
    ...
    if(reload)
    {
        uint32 val = sConfig.GetIntDefault("LogonZone", LOGON_ZONE_DEVELOPMENT);
        if(val!=m_configs[CONFIG_LOGON_ZONE])
            sLog.outError("LogonZone option can't be changed at Quadd.conf reload, using current value (%u).",m_configs[CONFIG_LOGON_ZONE]);
    }
    else
        m_configs[CONFIG_LOGON_ZONE] = sConfig.GetIntDefault("LogonZone", LOGON_ZONE_DEVELOPMENT);
    ...
    m_configs[CONFIG_CHARACTERS_PER_LOGON] = sConfig.GetIntDefault("CharactersPerLogon", 10);
    if(m_configs[CONFIG_CHARACTERS_PER_LOGON] < 1 || m_configs[CONFIG_CHARACTERS_PER_LOGON] > 10)
    {
        sLog.outError("CharactersPerLogon (%i) must be in range 1..10. Set to 10.",m_configs[CONFIG_CHARACTERS_PER_LOGON]);
        m_configs[CONFIG_CHARACTERS_PER_LOGON] = 10;
    }

    // must be after CONFIG_CHARACTERS_PER_LOGON
    m_configs[CONFIG_CHARACTERS_PER_ACCOUNT] = sConfig.GetIntDefault("CharactersPerAccount", 50);
    if(m_configs[CONFIG_CHARACTERS_PER_ACCOUNT] < m_configs[CONFIG_CHARACTERS_PER_LOGON])
    {
        sLog.outError("CharactersPerAccount (%i) can't be less than CharactersPerLogon (%i).",m_configs[CONFIG_CHARACTERS_PER_ACCOUNT],m_configs[CONFIG_CHARACTERS_PER_LOGON]);
        m_configs[CONFIG_CHARACTERS_PER_ACCOUNT] = m_configs[CONFIG_CHARACTERS_PER_LOGON];
    }

    m_configs[CONFIG_HEROIC_CHARACTERS_PER_LOGON] = sConfig.GetIntDefault("HeroicCharactersPerLogon", 1);
    if(int32(m_configs[CONFIG_HEROIC_CHARACTERS_PER_LOGON]) < 0 || m_configs[CONFIG_HEROIC_CHARACTERS_PER_LOGON] > 10)
    {
        sLog.outError("HeroicCharactersPerLogon (%i) must be in range 0..10. Set to 1.",m_configs[CONFIG_HEROIC_CHARACTERS_PER_LOGON]);
        m_configs[CONFIG_HEROIC_CHARACTERS_PER_LOGON] = 1;
    }
    ...
    m_configs[CONFIG_DECLINED_NAMES_USED] =
        (m_configs[CONFIG_LOGON_ZONE] == LOGON_ZONE_RUSSIAN) ? true : sConfig.GetBoolDefault("DeclinedNames", false);
...
void World::SetInitialWorldSettings()
{
    ...
    ///- Update the logon entry in the database with the logon type from the config file
    //No SQL injection as values are treated as integers

    // not send custom type LOGON_FFA_PVP to logon list
    uint32 server_type = IsFFAPvPLogon() ? LOGON_TYPE_PVP : getConfig(CONFIG_GAME_TYPE);
    uint32 logon_zone = getConfig(CONFIG_LOGON_ZONE);
    LoginDatabase.PExecute("UPDATE logonlist SET icon = %u, timezone = %u WHERE id = '%d'", server_type, logon_zone, logonID);
    ...
    LoginDatabase.PExecute("INSERT INTO uptime (logonid, starttime, startstring, uptime) VALUES('%u', " I64FMTD ", '%s', 0)",
        logonID, uint64(m_startTime), isoDate);
...
void World::Update(uint32 diff)
{
    ...
    if (m_timers[WUPDATE_UPTIME].Passed())
    {
        ...
        m_timers[WUPDATE_UPTIME].Reset();
        LoginDatabase.PExecute("UPDATE uptime SET uptime = %u, maxplayers = %u WHERE logonid = %u AND starttime = " I64FMTD, tmpDiff, maxClientsNum, logonID, uint64(m_startTime));
    }
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= m_gameTime))
    {
        ...
        switch (step.script->command)
        {
            ...
            case SCRIPT_COMMAND_RESPAWN_GAMEOBJECT:
            {
                ...
                CellPair p(Quad::ComputeCellPair(summoner->GetPositionX(), summoner->GetPositionY()));
                Cell cell(p);
                cell.data.Part.reserved = ALL_DISTRICT;

                QuadCore::GameObjectWithDbGUIDCheck go_check(*summoner,step.script->datalong);
                QuadCore::GameObjectSearcher<QuadCore::GameObjectWithDbGUIDCheck> checker(summoner, go,go_check);
            ...
            case SCRIPT_COMMAND_OPEN_DOOR:
            {
                ...
                CellPair p(Quad::ComputeCellPair(caster->GetPositionX(), caster->GetPositionY()));
                Cell cell(p);
                cell.data.Part.reserved = ALL_DISTRICT;

                QuadCore::GameObjectWithDbGUIDCheck go_check(*caster,step.script->datalong);
                QuadCore::GameObjectSearcher<QuadCore::GameObjectWithDbGUIDCheck> checker(caster,door,go_check);
...
void World::SendWorldText(int32 string_id, ...)
{
    ...
    QuadCore::WorldWorldTextBuilder wt_builder(string_id, &ap);
    QuadCore::LocalizedPacketListDo<QuadCore::WorldWorldTextBuilder> wt_do(wt_builder);
...
void World::UpdateLogonCharCount(uint32 accountId)
{
    CharacterDatabase.AsyncPQuery(this, &World::_UpdateLogonCharCount, accountId,
        "SELECT COUNT(guid) FROM characters WHERE account = '%u'", accountId);
}

void World::_UpdateLogonCharCount(QueryResult *resultCharCount, uint32 accountId)
{
    if (resultCharCount)
    {
        ...
        LoginDatabase.PExecute("DELETE FROM logoncharacters WHERE acctid= '%d' AND logonid = '%d'", accountId, logonID);
        LoginDatabase.PExecute("INSERT INTO logoncharacters (numchars, acctid, logonid) VALUES (%u, %u, %u)", charCount, accountId, logonID);
    }
}
...
void World::SetPlayerLimit( int32 limit, bool needUpdate )
{
    ...
    if(db_update_need)
        LoginDatabase.PExecute("UPDATE logonlist SET allowedSecurityLevel = '%u' WHERE id = '%d'",uint8(GetPlayerSecurityLimit()),logonID);
}
