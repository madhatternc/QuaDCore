...
bool ChatHandler::HandleGPSCommand(const char* args)
{
    ...
    sLog.outDebug("Player %s GPS call for %s '%s' (%s: %u):",
        m_session ? GetNameLink().c_str() : GetQuadCoreString(LANG_CONSOLE_COMMAND),
...
bool ChatHandler::HandleTeleNameCommand(const char * args)
{
    ...
    else if (uint64 guid = objmgr.GetPlayerGUIDByName(name))
    {
        ...
        PSendSysMessage(LANG_TELEPORTING_TO, nameLink.c_str(), GetQuadCoreString(LANG_OFFLINE), tele->name.c_str());
