...
bool ChatHandler::HandleGameObjectAddCommand(const char* args)
{
    ...
    sLog.outDebug(GetQuadCoreString(LANG_GAMEOBJECT_CURRENT), goI->name, db_lowGUID, x, y, z, o);
        ...
        if (factionEntry)
        {
            ...
            if(loc < MAX_LOCALE)
            {
                ...
                if (repState)                               // and then target!=NULL also
                {
                    ...
                    std::string rankName = GetQuadCoreString(ReputationRankStrIndex[rank]);
...
bool ChatHandler::HandlePInfoCommand(const char* args)
{
    ...
    PSendSysMessage(LANG_PINFO_ACCOUNT, (target?"":GetQuadCoreString(LANG_OFFLINE)), nameLink.c_str(), GUID_LOPART(targetGUID), username.c_str(), accId, security, last_ip.c_str(), last_login.c_str(), latency);
...
void ChatHandler::ShowTicket(uint64 guid, char const* text, char const* time)
{
    ...
    if(!objmgr.GetPlayerNameByGUID(guid,name))
        name = GetQuadString(LANG_UNKNOWN);
...
bool ChatHandler::HandleTicketCommand(const char* args)
{
    ...
    if (!px)
    {
        ...
        PSendSysMessage(LANG_COMMAND_TICKETCOUNT, count, accept ?  GetQuadString(LANG_ON) : GetQuadString(LANG_OFF));
        return true;
    }
