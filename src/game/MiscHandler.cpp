...
void WorldSession::HandleWhoOpcode( WorldPacket & recv_data )
{
    ...
    // client send in case not set max level value 100 but quadcore support 255 max level,
...
void WorldSession::HandleLogoutRequestOpcode( WorldPacket & /*recv_data*/ )
{
    ...
    //instant logout in taverns/cities or on taxi or for admins, gm's, mod's if its enabled in quadcore.conf
...
void WorldSession::HandleLogonStateRequestOpcode( WorldPacket & recv_data )
{
    CHECK_PACKET_SIZE(recv_data, 4);

    sLog.outDebug("CMSG_LOGON_SPLIT");

    uint32 unk;
    std::string split_date = "01/01/01";
    recv_data >> unk;

    WorldPacket data(SMSG_LOGON_SPLIT, 4+4+split_date.size()+1);
    data << unk;
    data << uint32(0x00000000);                             // logon split state
    // split states:
    // 0x0 logon normal
    // 0x1 logon split
    // 0x2 logon split pending
    data << split_date;
    SendPacket(&data);
