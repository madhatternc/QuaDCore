    ...
    if(m_formalCasterGUID == m_target->GetGUID())
    {
        ...
        if( !caster->hasUnitState(UNIT_STAT_ISOLATED) )
        {
            ...
            switch(m_areaAuraType)
            {
                ...
                {
                    Quad::AnyFriendlyUnitInObjectRangeCheck u_check(caster, caster, m_radius);
                    Quad::UnitListSearcher<Quad::AnyFriendlyUnitInObjectRangeCheck> searcher(caster, targets, u_check);
                    caster->VisitNearbyObject(m_radius, searcher);
                    break;
                }
                case AREA_AURA_ENEMY:
                {
                    Quad::AnyAoETargetUnitInObjectRangeCheck u_check(caster, caster, m_radius); // No GetCharmer in searcher
                    Quad::UnitListSearcher<Quad::AnyAoETargetUnitInObjectRangeCheck> searcher(caster, targets, u_check);
                    caster->VisitNearbyObject(m_radius, searcher);
                    break;
                }
...
void AuraEffect::HandleFeignDeath(bool apply, bool Real)
{
    ...
    if( apply )
    {
        ...
        */
        ...
        std::list<Unit*> targets;
        Quad::AnyUnfriendlyUnitInObjectRangeCheck u_check(m_target, m_target, World::GetMaxVisibleDistance());
