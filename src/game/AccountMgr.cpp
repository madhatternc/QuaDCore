...
AccountOpResult AccountMgr::CreateAccount(std::string username, std::string password)
{
    ...
    LoginDatabase.Execute("INSERT INTO logoncharacters (logonid, acctid, numchars) SELECT logonlist.id, account.id, 0 FROM logonlist,account LEFT JOIN logoncharacters ON acctid=account.id WHERE acctid IS NULL");
...
AccountOpResult AccountMgr::DeleteAccount(uint32 accid)
{
    ...
    if (result)
    {
        do
        {
            ...
            Player::DeleteFromDB(guid, accid, false);       // no need to update logon characters
        } while (result->NextRow());

        delete result;
    }

    // table logon specific but common for all characters of account for logon
    ...
    bool res =
        ...
        LoginDatabase.PExecute("DELETE FROM logoncharacters WHERE acctid='%d'", accid);
