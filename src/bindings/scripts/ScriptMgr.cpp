...
void LoadDatabase()
{
    ...
    if (dbstring.empty() )
    {
        error_log("QSCR: Missing world database info from configuration file. Load database aborted.");
        return;
    }

    //Initialize connection to DB
    if (!dbstring.empty() && TScriptDB.Initialize(dbstring.c_str()) )
        outstring_log("QSCR: QuadScript database: %s",dbstring.c_str());
    else
    {
        error_log("QSCR: Unable to connect to Database. Load database aborted.");
        return;
    }
    ...
    if (result)
    {
        ...
        outstring_log("QSCR: Database version is: %s", fields[0].GetString());
        outstring_log("");
        delete result;

    }else
    {
        error_log("QSCR: Missing `version.script_version` information.");
        outstring_log("");
    }
    ...
    // Load EventAI Text
    outstring_log("QSCR: Loading EventAI Texts...");
    LoadQuadStrings(TScriptDB,"creature_ai_texts",-1,1+(TEXT_SOURCE_RANGE));

    // Gather Additional data from EventAI Texts
    result = TScriptDB.PQuery("SELECT entry, sound, type, language, emote FROM creature_ai_texts");

    outstring_log("QSCR: Loading EventAI Texts additional data...");
    if (result)
    {
        ...
        do
        {
            ...
            if (i >= 0)
            {
                error_db_log("QSCR: Entry %i in table `creature_ai_texts` is not a negative value.",i);
                continue;
            }

            if (i <= TEXT_SOURCE_RANGE)
            {
                error_db_log("QSCR: Entry %i in table `creature_ai_texts` is out of accepted entry range for table.",i);
                continue;
            }
            ...
            if (temp.SoundId)
            {
                if (!GetSoundEntriesStore()->LookupEntry(temp.SoundId))
                    error_db_log("QSCR: Entry %i in table `creature_ai_texts` has soundId %u but sound does not exist.",i,temp.SoundId);
            }

            if (!GetLanguageDescByID(temp.Language))
                error_db_log("QSCR: Entry %i in table `creature_ai_texts` using Language %u but Language does not exist.",i,temp.Language);

            if (temp.Type > CHAT_TYPE_ZONE_YELL)
                error_db_log("QSCR: Entry %i in table `creature_ai_texts` has Type %u but this Chat Type does not exist.",i,temp.Type);
        ...
        outstring_log("");
        outstring_log(">> QSCR: Loaded %u additional EventAI Texts data.", count);
    }else
    {
        barGoLink bar(1);
        bar.step();
    ...
    // Load Script Text
    outstring_log("QSCR: Loading Script Texts...");
    ...
    outstring_log("QSCR: Loading Script Texts additional data...");
    if (result)
    {
        barGoLink bar(result->GetRowCount());
        uint32 count = 0;
        ...
        do
        {
            ...
            if (i >= 0)
            {
                error_db_log("QSCR: Entry %i in table `script_texts` is not a negative value.",i);
                continue;
            }

            if (i > TEXT_SOURCE_RANGE || i <= TEXT_SOURCE_RANGE*2)
            {
                error_db_log("QSCR: Entry %i in table `script_texts` is out of accepted entry range for table.",i);
                continue;
            }
            ...
            {
                if (!GetSoundEntriesStore()->LookupEntry(temp.SoundId))
                    error_db_log("QSCR: Entry %i in table `script_texts` has soundId %u but sound does not exist.",i,temp.SoundId);
            }

            if (!GetLanguageDescByID(temp.Language))
                error_db_log("QSCR: Entry %i in table `script_texts` using Language %u but Language does not exist.",i,temp.Language);

            if (temp.Type > CHAT_TYPE_ZONE_YELL)
                error_db_log("QSCR: Entry %i in table `script_texts` has Type %u but this Chat Type does not exist.",i,temp.Type);
        ...
        outstring_log("");
        outstring_log(">> QSCR: Loaded %u additional Script Texts data.", count);
    }else
    {
        barGoLink bar(1);
        bar.step();
    ...
    // Load Custom Text
    outstring_log("QSCR: Loading Custom Texts...");
    ...
    outstring_log("QSCR: Loading Custom Texts additional data...");
    if (result)
    {
        barGoLink bar(result->GetRowCount());
        uint32 count = 0;
        ...
        do
        {
            ...
            if (i >= 0)
            {
                error_db_log("QSCR: Entry %i in table `custom_texts` is not a negative value.",i);
                continue;
            }

            if (i > TEXT_SOURCE_RANGE*2 || i <= TEXT_SOURCE_RANGE*3)
            {
                error_db_log("QSCR: Entry %i in table `custom_texts` is out of accepted entry range for table.",i);
                continue;
            }
            ...
            {
                if (!GetSoundEntriesStore()->LookupEntry(temp.SoundId))
                    error_db_log("QSCR: Entry %i in table `custom_texts` has soundId %u but sound does not exist.",i,temp.SoundId);
            }

            if (!GetLanguageDescByID(temp.Language))
                error_db_log("QSCR: Entry %i in table `custom_texts` using Language %u but Language does not exist.",i,temp.Language);

            if (temp.Type > CHAT_TYPE_ZONE_YELL)
                error_db_log("QSCR: Entry %i in table `custom_texts` has Type %u but this Chat Type does not exist.",i,temp.Type);
    ...
    outstring_log("QSCR: Loading Script Waypoints for %u creature(s)...", uiCreatureCount);
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (!pCInfo)
            {
                error_db_log("QSCR: DB table script_waypoint has waypoint for non-existant creature entry %u", pTemp.m_uiCreatureEntry);
                continue;
            }

            if (!pCInfo->ScriptID)
                error_db_log("QSCR: DB table script_waypoint has waypoint for creature entry %u, but creature does not have ScriptName defined and then useless.", pTemp.m_uiCreatureEntry);
...
void ScriptsInit(char const* cfg_file = "quadcore.conf")
{
    ...
    {
        CanLoadDB = false;
        error_log("QSCR: Unable to open configuration file. Database will be unaccessible. Configuration values will use default.");
    }
    else outstring_log("QSCR: Using configuration file %s",cfg_file);
    ...
    outstring_log("QSCR: Loading C++ scripts");
    barGoLink bar(1);
    bar.step();
    outstring_log("");
...
void DoScriptText(int32 textEntry, WorldObject* pSource, Unit* target)
{
    if (!pSource)
    {
        error_log("QSCR: DoScriptText entry %i, invalid Source pointer.",textEntry);
        return;
    }

    if (textEntry >= 0)
    {
        error_log("QSCR: DoScriptText with source entry %u (TypeId=%u, guid=%u) attempts to process text entry %i, but text entry must be negative.",pSource->GetEntry(),pSource->GetTypeId(),pSource->GetGUIDLow(),textEntry);
        return;
    }
    ...
    if (i == TextMap.end())
    {
        error_log("QSCR: DoScriptText with source entry %u (TypeId=%u, guid=%u) could not find text entry %i.",pSource->GetEntry(),pSource->GetTypeId(),pSource->GetGUIDLow(),textEntry);
        return;
    }

    debug_log("QSCR: DoScriptText: text entry=%i, Sound=%u, Type=%u, Language=%u, Emote=%u",textEntry,(*i).second.SoundId,(*i).second.Type,(*i).second.Language,(*i).second.Emote);
    ...
    if((*i).second.SoundId)
    {
        if( GetSoundEntriesStore()->LookupEntry((*i).second.SoundId) )
        {
            ...
            error_log("QSCR: DoScriptText entry %i tried to process invalid sound id %u.",textEntry,(*i).second.SoundId);
    }
    ...
    if((*i).second.Emote)
    {
        if (pSource->GetTypeId() == TYPEID_UNIT || pSource->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            error_log("QSCR: DoScriptText entry %i tried to process emote for invalid TypeId (%u).",textEntry,pSource->GetTypeId());
    }
        ...
        case CHAT_TYPE_WHISPER:
            {
                ...
                else error_log("QSCR: DoScriptText entry %i cannot whisper without target unit (TYPEID_PLAYER).", textEntry);
            }break;
...
{
    debug_log("QSCR: Gossip selection, sender: %d, action: %d",sender, action);
...
{
    debug_log("QSCR: Gossip selection with code, sender: %d, action: %d",sender, action);
