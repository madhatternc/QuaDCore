...
*Copyright (C) 2006 - 2009  Quad Script <http://www.quadcore.org/>
...
bool ItemUse_item_flying_machine(Player *player, Item* _Item, SpellCastTargets const& targets)
{
    ...
    debug_log("QSCR: Player attempt to use item %u, but did not meet riding requirement",itemId);
...
{
    if (targets.getUnitTarget() && targets.getUnitTarget()->GetTypeId()==TYPEID_UNIT && targets.getUnitTarget()->isDead() &&
        targets.getUnitTarget()->GetEntry() == 25623)
        return false;
...
{
    if (targets.getUnitTarget() && targets.getUnitTarget()->GetTypeId()==TYPEID_UNIT && (targets.getUnitTarget()->GetMaxHealth() < 35) &&
        targets.getUnitTarget()->GetEntry() == 26411)
        return false;
...
{
    if (targets.getUnitTarget() && targets.getUnitTarget()->GetTypeId()==TYPEID_UNIT && (targets.getUnitTarget()->GetMaxHealth() < 35) &&
        targets.getUnitTarget()->GetEntry() == 26283)
        return false;
...
{
    if (targets.getUnitTarget() && targets.getUnitTarget()->GetTypeId()==TYPEID_UNIT && (targets.getUnitTarget()->GetMaxHealth() < 35) &&
        targets.getUnitTarget()->GetEntry() == 26926)
        return false;
...
/*#####
# item_scepter_of_domination
/#####*/

bool ItemUse_item_scepter_of_domination(Player *player, Item* _Item, SpellCastTargets const& targets)
{
    if (targets.getUnitTarget() && targets.getUnitTarget()->GetTypeId()==TYPEID_UNIT &&
        targets.getUnitTarget()->GetEntry() == 28931)
        return false;
...
{
    if (targets.getUnitTarget() && targets.getUnitTarget()->GetTypeId()==TYPEID_UNIT &&
        targets.getUnitTarget()->GetEntry() == 32201)
        return false;
...
void AddSC_item_scripts()
{
    ...
    newscript = new Script;
    newscript->Name = "item_scepter_of_domination";
    newscript->pItemUse = &ItemUse_item_scepter_of_domination;
    newscript->RegisterSelf();
