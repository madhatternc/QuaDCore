...
void npc_escortAI::EnterCombat(Unit *who)
{
    if (IsBeingEscorted)
    {
        ...
        debug_log("QSCR: EscortAI has entered combat and stored last location.");
    }
}
...
void npc_escortAI::EnterEvadeMode()
{
    ...
    if (IsBeingEscorted)
    {
        debug_log("QSCR: EscortAI has left combat and is now returning to last point.");
        Returning = true;
...
void npc_escortAI::UpdateAI(const uint32 diff)
{
        ...
        if (WaitTimer <= diff)
    {
        if (ReconnectWP)
        {
            ...
            if( !IsOnHold )
            {
                    if (CurrentWP != WaypointList.end())
                    {
                        ...
                        debug_log("QSCR: EscortAI Reconnect WP is: %u, %f, %f, %f", CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z);
                        ...
                        debug_log("QSCR: EscortAI Reconnected to end of WP list");
            }
        }
        ...
        if (CurrentWP == WaypointList.end())
        {
            if(DespawnAtEnd)
            {
                debug_log("QSCR: EscortAI reached end of waypoints");
            ...
            {
                debug_log("QSCR: EscortAI reached end of waypoints with Despawn off");
                IsBeingEscorted = false;
                WaitTimer = 0;
                return;
            }
        ...
        {
            m_creature->GetMotionMaster()->MovePoint(CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z );
            debug_log("QSCR: EscortAI Next WP is: %u, %f, %f, %f", CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z);
            WaitTimer = 0;
        }
    }else WaitTimer -= diff;
    }
        ...
        if (PlayerTimer < diff)
    {
        ...
        if (DespawnAtFar && (!p || m_creature->GetDistance(p) > GetMaxPlayerDistance()))
        {
            ...
            debug_log("QSCR: EscortAI Evaded back to spawn point because player was to far away or not found");
...
void npc_escortAI::MovementInform(uint32 type, uint32 id)
{
    ...
    if (id == WP_LAST_POINT)
    {
        debug_log("QSCR: EscortAI has returned to original position before combat");
        ReconnectWP = true;
        Returning = false;
        WaitTimer = 1;
    ...
    else
    {
        ...
        debug_log("QSCR: EscortAI Waypoint %d reached", CurrentWP->id);
...
void npc_escortAI::SetRun(bool bRun)
{
    if (bRun)
    {
        if (!bIsRunning)
        {
            ...
            debug_log("QSCR: EscortAI attempt to set run mode, but is already running.");
    }
    else
    {
        if (bIsRunning)
        {
            ...
            debug_log("QSCR: EscortAI attempt to set walk mode, but is already walking.");
    }
}
...
void npc_escortAI::Start(bool bAttack, bool bDefend, bool bRun, uint64 pGUID)
{
    ...
    if (IsBeingEscorted)
    {
        error_log("QSCR: EscortAI attempt to Start while already escorting");
        return;
    }

    if (WaypointList.empty())
    {
        error_db_log("QSCR: EscortAI Start with 0 waypoints (possible missing entry in script_waypoint)");
        return;
    }
    ...
    debug_log("QSCR: EscortAI started with %d waypoints. Attack = %d, Defend = %d, Run = %d, PlayerGUID = %d", WaypointList.size(), Attack, Defend, bIsRunning, PlayerGUID);
    ...
    //Start WP
    m_creature->GetMotionMaster()->MovePoint(CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z );
    debug_log("QSCR: EscortAI Next WP is: %d, %f, %f, %f", CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z);
