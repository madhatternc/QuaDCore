...
struct QUAD_DLL_DECL Mob_EventAI : public ScriptedAI
{
    ...
    bool ProcessEvent(EventHolder& pHolder, Unit* pActionInvoker = NULL)
    {
        ...
        case EVENT_T_TIMER:
            {
                ...
                {
                    if (EAI_ErrorLevel > 0)
                        error_db_log("QSCR: Creature %u using Event %u (Type = %u) has RandomMax < RandomMin. Event repeating disabled.", m_creature->GetEntry(), pHolder.Event.event_id, pHolder.Event.event_type);
        ...
        default:
            if (EAI_ErrorLevel > 0)
                error_db_log("QSCR: Creature %u using Event %u has invalid Event Type(%u), missing from ProcessEvent() Switch.", m_creature->GetEntry(), pHolder.Event.event_id, pHolder.Event.event_type);
            break;
        }
        ...
        case ACTION_T_CAST:
            {
                ...
                if (canCast)
                {
                    ...
                    if (tSpell)
                    {
                        ...
                        error_db_log("QSCR: EventAI event %d creature %d attempt to cast spell that doesn't exist %d", EventId, m_creature->GetEntry(), param1);
                }
            }
            break;
        case ACTION_T_SUMMON:
            {
                ...
                {
                    if (EAI_ErrorLevel > 0)
                        error_db_log( "QSCR: EventAI failed to spawn creature %u. Spawn event %d is on creature %d", param1, EventId, m_creature->GetEntry());
                }
        ...
        case ACTION_T_INC_PHASE:
            {
                ...
                if (Phase > 31)
                    if (EAI_ErrorLevel > 0)
                        error_db_log( "QSCR: Event %d incremented Phase above 31. Phase mask cannot be used with phases past 31. CreatureEntry = %d", EventId, m_creature->GetEntry());
            }
            break;
        case ACTION_T_EVADE:
            {
        ...
        case ACTION_T_RANDOM_PHASE_RANGE:
            {
                if (param2 > param1)
                {
                    ...
                    error_db_log( "QSCR: ACTION_T_RANDOM_PHASE_RANGE cannot have Param2 <= Param1. Divide by Zero. Event = %d. CreatureEntry = %d", EventId, m_creature->GetEntry());
            }
            break;
        case ACTION_T_SUMMON_ID:
            {
                ...
                {
                    if (EAI_ErrorLevel > 0)
                        error_db_log( "QSCR: EventAI failed to spawn creature %u. Summon map index %u does not exist. EventID %d. CreatureID %d", param1, param3, EventId, m_creature->GetEntry());
                    return;
                }
                ...
                {
                    if (EAI_ErrorLevel > 0)
                        error_db_log( "QSCR: EventAI failed to spawn creature %u. EventId %d.Creature %d", param1, EventId, m_creature->GetEntry());
                }
                else if (param2 != TARGET_T_SELF && target)
                pCreature->AI()->AttackStart(target);
            }
        ...
        case ACTION_T_SET_INST_DATA:
            {
                ...
                {
                    if (EAI_ErrorLevel > 0)
                        error_db_log("QSCR: Event %d attempt to set instance data without instance script. Creature %d", EventId, m_creature->GetEntry());
                    return;
                }
        ...
        case ACTION_T_SET_INST_DATA64:
            {
                ...
                {
                    if (EAI_ErrorLevel > 0)
                        error_db_log("QSCR: Event %d attempt to set instance data64 but Target == NULL. Creature %d", EventId, m_creature->GetEntry());
                    return;
                }
                ...
                {
                    if (EAI_ErrorLevel > 0)
                        error_db_log("QSCR: Event %d attempt to set instance data64 without instance script. Creature %d", EventId, m_creature->GetEntry());
                    return;
                }
        ...
        case ACTION_T_UPDATE_TEMPLATE:
            {
                ...
                {
                    if (EAI_ErrorLevel > 0)
                        error_db_log("QSCR: Event %d ACTION_T_UPDATE_TEMPLATE call with param1 == current entry. Creature %d", EventId, m_creature->GetEntry());
                    return;
                }
        ...
        case ACTION_T_DIE:
            {
                ...
                {
                    if (EAI_ErrorLevel > 0)
                        error_db_log("QSCR: Event %d ACTION_T_DIE on dead creature. Creature %d", EventId, m_creature->GetEntry());
                    return;
                }
        ...
        case ACTION_T_ZONE_COMBAT_PULSE:
            {
                ...
                {
                    if (EAI_ErrorLevel > 0)
                        error_db_log("QSCR: Event %d ACTION_T_ZONE_COMBAT_PULSE on creature out of combat or in non-dungeon map. Creature %d", EventId, m_creature->GetEntry());
                    return;
                }
            ...
            case ACTION_T_SUMMON_GO:
            {
                ...
                {
                    if (EAI_ErrorLevel > 0)
                        error_db_log( "QSCR: EventAI failed to spawn object %u. Spawn event %d is on creature %d", param1, EventId, m_creature->GetEntry());
                }
            }
            break;
                ...
                case EVENT_T_TIMER_OOC:
                    {
                        ...
                        }else if ((*i).Event.event_param2 > (*i).Event.event_param1)
                        {
                            ...
                            error_db_log("QSCR: Creature %u using Event %u (Type = %u) has InitialMax < InitialMin. Event disabled.", m_creature->GetEntry(), (*i).Event.event_id, (*i).Event.event_type);
                    }
                    break;
                //default:
...
CreatureAI* GetAI_Mob_EventAI(Creature *pCreature)
{
    ...
    if (CreatureEvents != EventAI_Event_Map.end())
    {
        ...
        for (i = (*CreatureEvents).second.begin(); i != (*CreatureEvents).second.end(); ++i)
        {
         ...
         {
             if (EAI_ErrorLevel > 1)
                 error_db_log("QSCR: CreatureId has events but no events added to list because of instance flags.", pCreature->GetEntry());
         }
      }
     else
      {
          if (EAI_ErrorLevel > 1)
             error_db_log("QSCR: EventMap for Creature %u is empty but creature is using Mob_EventAI.", pCreature->GetEntry());
      }
