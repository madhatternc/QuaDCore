...
struct QUAD_DLL_DECL boss_shade_of_akamaAI : public ScriptedAI
{
    ...
    void IncrementDeathCount(uint64 guid = 0)               // If guid is set, will remove it from list of sorcerer
    {
        ...
        debug_log("QSCR: Increasing Death Count for Shade of Akama encounter");
        ++DeathCount;
    ...
    void FindChannelers()
    {
        ...
        Quad::AllCreaturesOfEntryInRange check(m_creature, CREATURE_CHANNELER, 50);
        Quad::CreatureListSearcher<Quad::AllCreaturesOfEntryInRange> searcher(m_creature, ChannelerList, check);
        ...
        if(!ChannelerList.empty())
        {
            for(std::list<Creature*>::iterator itr = ChannelerList.begin(); itr != ChannelerList.end(); ++itr)
            {
                ...
                debug_log("QSCR: Shade of Akama Grid Search found channeler %u. Adding to list", (*itr)->GetGUID());
            }
        }
