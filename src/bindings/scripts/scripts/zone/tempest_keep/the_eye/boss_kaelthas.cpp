...
struct QUAD_DLL_DECL boss_kaelthasAI : public ScriptedAI
{
    ...
    void StartEvent()
    {
        ...
        {
            error_log("QSCR: Kael'Thas One or more advisors missing, Skipping Phases 1-3");
            DoYell("QSCR: Kael'Thas One or more advisors missing, Skipping Phases 1-3", LANG_UNIVERSAL, NULL);
            ...
            case 2:
            {
                ...
                if (PhaseSubphase == 1)
                {
                    ...
                    for (uint32 i = 0; i < 7; ++i)
                    {
                        ...
                        if (!Weapon)
                            error_log("SQSCR: Kael'thas weapon %i could not be spawned", i);
                        else
                        {
            ...
            case 3:
            {
                if (PhaseSubphase == 0)
                {
                    ...
                    for (uint32 i = 0; i < 4; ++i)
                    {
                        ...
                        if (!Advisor)
                            error_log("QSCR: Kael'Thas Advisor %u does not exist. Possibly despawned? Incorrectly Killed?", i);
                        else if(Target)
                ...
                if(!InGravityLapse && !ChainPyros && Phase != 5)
                {
                    ...
                    if (MindControl_Timer < diff)
                    {
                        ...
                        for (uint32 i = 0; i < 3; i++)
                        {
                            ...
                            debug_log("QSCR: Kael'Thas mind control not supported.");
                            if(target)
