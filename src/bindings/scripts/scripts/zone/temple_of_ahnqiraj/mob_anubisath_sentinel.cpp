...
struct QUAD_DLL_DECL aqsentinelAI : public ScriptedAI
{
    ...
    {
        CellPair p(Quad::ComputeCellPair(nears->GetPositionX(), nears->GetPositionY()));
        Cell cell(p);
        ...
        Quad::CreatureListSearcher<NearbyAQSentinel> searcher(m_creature, assistList, u_check);
        TypeContainerVisitor<Quad::CreatureListSearcher<NearbyAQSentinel>, GridTypeMapContainer >  grid_creature_searcher(searcher);
