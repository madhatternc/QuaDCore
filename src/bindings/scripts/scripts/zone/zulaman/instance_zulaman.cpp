...
struct QUAD_DLL_DECL instance_zulaman : public ScriptedInstance
{
    ...
    const char* Save()
    {
        ...
        //error_log("QSCR: Zul'aman saved, %s.", data);
        return data;
    }
    ...
    void Load(const char* load)
    {
        ...
        //error_log("QSCR: Zul'aman loaded, %s.", ss.str().c_str());
        char dataHead; // S
        uint16 data1, data2, data3;
        ss >> dataHead >> data1 >> data2 >> data3;
        //error_log("QSCR: Zul'aman loaded, %d %d %d.", data1, data2, data3);
        if(dataHead == 'S')
        {
            BossKilled = data1;
            ChestLooted = data2;
            QuestMinute = data3;
        }else error_log("QSCR: Zul'aman: corrupted save data.");
    }
