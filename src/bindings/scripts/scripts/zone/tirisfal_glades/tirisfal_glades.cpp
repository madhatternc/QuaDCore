...
GameObject* SearchMausoleumGo(Unit *source, uint32 entry, float range)
{
    ...
    Quad::NearestGameObjectEntryInObjectRangeCheck go_check(*source, entry, range);
    Quad::GameObjectLastSearcher<Quad::NearestGameObjectEntryInObjectRangeCheck> searcher(source, pGo, go_check);

    TypeContainerVisitor<Quad::GameObjectLastSearcher<Quad::NearestGameObjectEntryInObjectRangeCheck>, GridTypeMapContainer> go_searcher(searcher);
