    ...
    boss_selin_fireheartAI(Creature* c) : ScriptedAI(c)
    {
        ...
        if (pInstance)
        {
            ...
            for(uint8 i = 0; i < size; ++i)
            {
                ...
                debug_log("QSCR: Selin: Adding Fel Crystal %u to list", guid);
    ...
    void Reset()
    {
        if (pInstance)
        {
            ...
            for(std::list<uint64>::iterator itr = Crystals.begin(); itr != Crystals.end(); ++itr)
            {
                ...
                {
                    if(!pUnit->isAlive())
                        ((Creature*)pUnit)->Respawn();      // Let QuadCore handle setting death state, etc.
    ...
    void MovementInform(uint32 type, uint32 id)
    {
        if (type == POINT_MOTION_TYPE && id == 1)
        {
            ...
            else
            {
                ...
                error_log("QSCR: Selin Fireheart unable to drain crystal as the crystal is either dead or despawned");
                DrainingCrystal = false;
            }
        }
    }
