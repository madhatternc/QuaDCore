...
struct QUAD_DLL_DECL instance_dark_portal : public ScriptedInstance
{
    ...
    Player* GetPlayerInMap()
    {
        ...
        debug_log("QSCR: Instance Black Portal: GetPlayerInMap, but PlayerList is empty!");
        return NULL;
    }
    ...
    void UpdateBMWorldState(uint32 id, uint32 state)
    {
        ...
        }else debug_log("QSCR: Instance Black Portal: UpdateBMWorldState, but PlayerList is empty!");
    }
    ...
    void SetData(uint32 type, uint32 data)
    {
        ...
        if (!player)
        {
            debug_log("QSCR: Instance Black Portal: SetData (Type: %u Data %u) cannot find any player.", type, data);
            return;
        }
        ...
        case TYPE_MEDIVH:
            ...
            else
            {
                if (data == IN_PROGRESS)
                {
                    debug_log("QSCR: Instance Dark Portal: Starting event.");
                    InitWorldState();
                ...
                if (data == DONE)
                {
                    ...
                    if (Unit *medivh = Unit::GetUnit(*player,MedivhGUID))
                    {
                        debug_log("QSCR: Instance Dark Portal: Event completed.");
    ...
    Unit* SummonedPortalBoss(Unit* source)
    {
        ...
        debug_log("QSCR: Instance Dark Portal: Summoning rift boss entry %u.",entry);
        ...
        debug_log("QSCR: Instance Dark Portal: what just happened there? No boss, no loot, no fun...");
        return NULL;
    }
    ...
    void DoSpawnPortal()
    {
        ...
        if (Unit *medivh = Unit::GetUnit(*player,MedivhGUID))
        {
            ...
            if (tmp >= CurrentRiftId)
                tmp++;
                    debug_log("QSCR: Instance Dark Portal: Creating Time Rift at locationId %i (old locationId was %u).",tmp,CurrentRiftId);
