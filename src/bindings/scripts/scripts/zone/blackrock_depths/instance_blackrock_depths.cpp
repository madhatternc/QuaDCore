...
struct QUAD_DLL_DECL instance_blackrock_depths : public ScriptedInstance
{
    ...
    Player* GetPlayerInMap()
    {
        ...
        debug_log("QSCR: Instance Blackrock Depths: GetPlayerInMap, but PlayerList is empty!");
        return NULL;
    }
    ...
    void SetData(uint32 type, uint32 data)
    {
        ...
        if (!player)
        {
            debug_log("QSCR: Instance Blackrock Depths: SetData (Type: %u Data %u) cannot find any player.", type, data);
            return;
        }

        debug_log("QSCR: Instance Blackrock Depths: SetData update (Type: %u Data %u)", type, data);
