...
#define DRAGON_LOGON_Z  53.079
#define DEMON_LOGON_Z   -74.558
...
struct QUAD_DLL_DECL boss_sathrovarrAI : public ScriptedAI
{
    ...
    void JustDied(Unit *killer)
    {
        ...
        m_creature->Relocate(m_creature->GetPositionX(), m_creature->GetPositionY(), DRAGON_LOGON_Z, m_creature->GetOrientation());
        TeleportAllPlayersBack();
        if(Unit *Kalecgos = Unit::GetUnit(*m_creature, KalecgosGUID))
        {
            ((boss_kalecgosAI*)((Creature*)Kalecgos)->AI())->TalkTimer = 1;
    ...
    void TeleportAllPlayersBack()
    {
        ...
        for(i = PlayerList.begin(); i != PlayerList.end(); ++i)
            if(Player* i_pl = i->getSource())
                if(i_pl->HasAura(AURA_SPECTRAL_LOGON))
                    i_pl->RemoveAurasDueToSpell(AURA_SPECTRAL_LOGON);
    }
        ...
        if(CheckTimer < diff)
         {
             ...
             if(!isBanished && (m_creature->GetHealth()*100)/m_creature->GetMaxHealth() < 1)
             {
                 ...
                 {
                     error_log("QSCR: Didn't find Shathrowar. Kalecgos event reseted.");
                     EnterEvadeMode();
                     return;
                 }
             }
