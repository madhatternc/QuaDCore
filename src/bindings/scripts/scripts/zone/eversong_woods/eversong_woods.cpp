...
bool GOHello_go_second_trial(Player *player, GameObject* _GO)
{
    ...
    CellPair p(Quad::ComputeCellPair(_GO->GetPositionX(), _GO->GetPositionY()));
    Cell cell(p);
    ...
    Quad::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*_GO, MASTER_KELERUN_BLOODMOURN, true, 30);
    Quad::CreatureLastSearcher<Quad::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(player, event_controller, u_check);
    TypeContainerVisitor<Quad::CreatureLastSearcher<Quad::NearestCreatureEntryWithLiveStateInObjectRangeCheck>, GridTypeMapContainer >  grid_unit_searcher(searcher);
