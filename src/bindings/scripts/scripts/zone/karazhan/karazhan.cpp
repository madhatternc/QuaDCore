...
struct QUAD_DLL_DECL npc_barnesAI : public npc_escortAI
{
    ...
    {
        debug_log("QSCR: Barnes Opera Event - Introduction complete - preparing encounter %d", Event);
        uint8 index = 0;
        uint8 count = 0;
        switch(Event)
        {
...
bool GossipSelect_npc_barnes(Player *player, Creature *_Creature, uint32 sender, uint32 action)
{
    switch(action)
    {
        ...
        case GOSSIP_ACTION_INFO_DEF+3:
            ...
            outstring_log("QSCR: player (GUID %i) manually set Opera event to EVENT_OZ",player->GetGUID());
            break;
        case GOSSIP_ACTION_INFO_DEF+4:
            player->CLOSE_GOSSIP_MENU();
            ((npc_barnesAI*)_Creature->AI())->Event = EVENT_HOOD;
            outstring_log("QSCR: player (GUID %i) manually set Opera event to EVENT_HOOD",player->GetGUID());
            break;
        case GOSSIP_ACTION_INFO_DEF+5:
            player->CLOSE_GOSSIP_MENU();
            ((npc_barnesAI*)_Creature->AI())->Event = EVENT_RAJ;
            outstring_log("QSCR: player (GUID %i) manually set Opera event to EVENT_RAJ",player->GetGUID());
            break;
    }
