...
struct QUAD_DLL_DECL npc_commander_dawnforgeAI : public ScriptedAI
{
    ...
    Creature* SelectCreatureInGrid(uint32 entry, float range)
    {
        ...
        Quad::NearestCreatureEntryWithLiveStateInObjectRangeCheck creature_check(*m_creature, entry, true, range);
		Quad::CreatureLastSearcher<Quad::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(m_creature, pCreature, creature_check);
    ...
    bool CanStartEvent(Player *player)
    {
        ...
        debug_log("QSCR: npc_commander_dawnforge event already in progress, need to wait.");
        return false;
    }
...
Creature* SearchDawnforge(Player *source, uint32 entry, float range)
{
    ...
    CellPair pair(Quad::ComputeCellPair(source->GetPositionX(), source->GetPositionY()));
    Cell cell(pair);
    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();

    Quad::NearestCreatureEntryWithLiveStateInObjectRangeCheck creature_check(*source, entry, true, range);
	Quad::CreatureLastSearcher<Quad::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(source, pCreature, creature_check);
