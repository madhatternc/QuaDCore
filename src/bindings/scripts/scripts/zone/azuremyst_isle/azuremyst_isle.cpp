...
bool GossipSelect_npc_susurrus(Player *player, Creature *_Creature, uint32 sender, uint32 action )
{
    if (action == GOSSIP_ACTION_INFO_DEF)
    {
        ...
        player->ActivateTaxiPathTo(nodes,11686);            //TaxiPath 506. Using invisible model, possible Quad must allow 0(from dbc) for cases like this.
    }
    return true;
}
...
struct QUAD_DLL_DECL npc_geezleAI : public ScriptedAI
{
    ...
    void DespawnNagaFlag(bool despawn)
    {
        ...
        Quad::AllGameObjectsWithEntryInGrid go_check(GO_NAGA_FLAG);
		Quad::GameObjectListSearcher<Quad::AllGameObjectsWithEntryInGrid> go_search(m_creature, FlagList, go_check);
        TypeContainerVisitor
            <Quad::GameObjectListSearcher<Quad::AllGameObjectsWithEntryInGrid>, GridTypeMapContainer> go_visit(go_search);
...
struct QUAD_DLL_DECL mob_nestlewood_owlkinAI : public ScriptedAI
{
