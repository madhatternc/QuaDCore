    ...
    void run ()
    {
        ...
        if (usera)
        {
            ...
            if (!Utility::u2ip (stringip, raip))
                sLog.outError ("QuadCore RA can not bind to ip %s", stringip.c_str ());
            else if (RAListenSocket.Bind (raip, raport))
                sLog.outError ("QuadCore RA can not bind to port %d on %s", raport, stringip.c_str ());
            else
            {
...
int Master::Run()
{
    ...
    // set server online
    LoginDatabase.PExecute("UPDATE logonlist SET color = 0, population = 0 WHERE id = '%d'",logonID);
...
//        if(Prio && (m_ServiceStatus == -1)/* need set to default process priority class in service mode*/)
    ...
    // set server offline
    LoginDatabase.PExecute("UPDATE logonlist SET color = 2 WHERE id = '%d'",logonID);
...
bool Master::_StartDB()
{
    ...
    ///- Get the logon Id from the configuration file
    logonID = sConfig.GetIntDefault("LogonID", 0);
    if(!logonID)
    {
        sLog.outError("Logon ID not defined in configuration file");
        return false;
    }
    sLog.outString("Logon running as logon ID %d", logonID);
...
/// Clear 'online' status for all accounts with characters in this logon
void Master::clearOnlineAccounts()
{
    // Cleanup online status for characters hosted at current logon
    /// \todo Only accounts with characters logged on *this* logon should have online status reset. Move the online column from 'account' to 'logoncharacters'?
    LoginDatabase.PExecute(
        "UPDATE account SET online = 0 WHERE online > 0 "
        "AND id IN (SELECT acctid FROM logoncharacters WHERE logonid = '%d')",logonID);
