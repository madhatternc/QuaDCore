...
const AuthHandler table[] =
{
    ...
    { LOGON_LIST,               STATUS_AUTHED,    &AuthSocket::_HandleLogonList         },
...
bool AuthSocket::_HandleLogonChallenge()
{
    ...
    if(result)
    {
        pkt << (uint8)LOGON_AUTH_ACCOUNT_BANNED;
        ...
        if( result )
        {
            ...
            if((*result)[2].GetUInt8() == 1)            // if ip is locked
            {
                ...
                if ( strcmp((*result)[3].GetString(),GetRemoteAddress().c_str()) )
                {
                    ...
                    pkt << (uint8) LOGON_AUTH_ACCOUNT_FREEZED;
                    locked=true;
                }
                else
                {
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if(!valid_version)
    {
        ...
        if(!pFile)
        {
            ...
            pkt << (uint8) AUTH_LOGON_CHALLENGE;
            pkt << (uint8) 0x00;
            pkt << (uint8) LOGON_AUTH_WRONG_BUILD_NUMBER;
        ...
        else                                                // have patch
        {
            ...
            uint8 data[2]={AUTH_LOGON_PROOF,LOGON_AUTH_UPDATE_CLIENT};
    ...
    {
        char data[4]={AUTH_LOGON_PROOF,LOGON_AUTH_NO_MATCH,3,0};
            ...
            if(QueryResult *loginfail = LoginDatabase.PQuery("SELECT id, failed_logins FROM account WHERE username = '%s'", _safelogin.c_str()))
            {
                ...
                if( failed_logins >= MaxWrongPassCount )
                {
                    ...
                    if(WrongPassBanType)
                    {
                        ...
                        LoginDatabase.PExecute("INSERT INTO account_banned VALUES ('%u',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','Quad logon','Failed login autoban',1)",
                    ...
                    else
                    {
                        ...
                        LoginDatabase.PExecute("INSERT INTO ip_banned VALUES ('%s',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','Quad logon','Failed login autoban')",
...
/// %Logon List command handler
bool AuthSocket::_HandleLogonList()
{
    DEBUG_LOG("Entering _HandleLogonList");
    if (ibuf.GetLength() < 5)
        return false;
    ...
    ///- Update logon list if need
    m_logonList.UpdateIfNeed();

    ///- Circle through logons in the LogonList and construct the return packet (including # of user characters in each logon)
    ByteBuffer pkt;
    pkt << (uint32) 0;
    pkt << (uint16) m_logonList.size();
    LogonList::LogonMap::const_iterator i;
    for( i = m_logonList.begin(); i != m_logonList.end(); i++ )
    {
        uint8 AmountOfCharacters;

        // No SQL injection. id of logon is controlled by the database.
        result = LoginDatabase.PQuery( "SELECT numchars FROM logoncharacters WHERE logonid = '%d' AND acctid='%u'",i->second.m_ID,id);
        if( result )
        {
            Field *fields = result->Fetch();
        ...
        pkt << i->second.icon;                             // logon type
        pkt << lock;                                       // if 1, then logon locked
        pkt << i->second.color;                            // if 2, then logon is offline
        pkt << i->first;
        pkt << i->second.address;
        pkt << i->second.populationLevel;
        pkt << AmountOfCharacters;
        pkt << i->second.timezone;                          // logon category
        pkt << (uint8) 0x2C;                                // unk, may be logon number/id?
    }
    pkt << (uint8) 0x10;
    pkt << (uint8) 0x00;

    ByteBuffer hdr;
    hdr << (uint8) LOGON_LIST;
    hdr << (uint16)pkt.size();
    hdr.append(pkt);

    SendBuf((char const*)hdr.contents(), hdr.size());

    // Set check field before possible relogin to logon
    _SetVSFields(rI);
    return true;
}
