...
PacketLog::PacketLog()
{
    ...
    if (sConfig.GetBoolDefault("LogLogon", false))
    {
        FILE *pFile = fopen("logon.log", "w+");
        fclose(pFile);
    }
...
void PacketLog::LogonHexDump(LogonPacket* data, uint32 socket, bool direction)
{
    if (!sConfig.GetBoolDefault("LogLogon", false))
        return;

    FILE *pFile;
    pFile = fopen("logon.log", "a");
    ...
    fclose(pFile);
    HexDump((char *)data->contents(), data->size(), "logon.log");
